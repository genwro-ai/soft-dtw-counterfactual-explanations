...
## New: Run with aeon datasets and VRNN NLL (latent_flow_ts integration)

You can now train and evaluate the LatentCF (Glacier) method using datasets provided by `latent_flow_ts.data.get_datasets`, and optionally compute VRNN negative log-likelihoods (NLL) for generated counterfactuals.

Example (binary datasets only):

```
python src/train_latentcf_with_get_datasets.py \
  --dataset TwoLeadECG \
  --output ./results/TwoLeadECG_latentcf_aeon.csv \
  --w-type local --w-value 0.5 --tau-value 0.5 \
  --use-vrnn \
  --vrnn-model-path ./results/TwoLeadECG_vrnn.pt
```

Notes:
- Datasets are loaded via `latent_flow_ts.data.get_datasets.get_data` and min–max normalized per-channel using train statistics.
- VRNN density uses the implementation from `latent_flow_ts.solver4`. If `--vrnn-model-path` exists, it is loaded; otherwise a new model is trained and saved there.
- The output CSV includes extra columns for VRNN NLLs: `vrnn_cf_nll_mean`, `vrnn_orig_nll_mean`, `vrnn_cf_nll_delta_mean`.