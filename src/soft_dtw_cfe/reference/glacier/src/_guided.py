...
try:
    from soft_dtw_cfe.reference.glacier.src.LIMESegment.Utils.explanations import LIMESegment
...
class ModifiedLatentCF:
        ...
        *,
        ...
        autoencoder : int, optional
            ...
            - if given, the autoencoder is expected to have `k` decoder layer and `k`
              encoding layers.
        """
        # Store optimizer config instead of instance to avoid variable binding issues
        if optimizer is None:
            self.optimizer_config_ = {"class_name": "Adam", "learning_rate": 1e-4}
        else:
            # Extract config from provided optimizer
            if hasattr(optimizer, 'get_config'):
                self.optimizer_config_ = optimizer.get_config()
                self.optimizer_config_["class_name"] = optimizer.__class__.__name__
            else:
                # Fallback for older TF versions
                self.optimizer_config_ = {"class_name": "Adam", "learning_rate": getattr(optimizer, 'learning_rate', 1e-4)}
    ...
    def _transform_sample(self, x, step_weights, pred_label):
        ...
        # Create a fresh optimizer for this sample to avoid variable binding issues
        if self.optimizer_config_["class_name"] == "Adam":
            optimizer = tf.optimizers.Adam(learning_rate=self.optimizer_config_["learning_rate"])
        else:
            # Fallback to Adam if unknown optimizer type
            optimizer = tf.optimizers.Adam(learning_rate=self.optimizer_config_.get("learning_rate", 1e-4))
...
def get_local_weights(
    ...
    if LIMESegment is None:
        raise ImportError(
            "LIMESegment is not available. Please install the LIMESegment dependency "
            "or run with step_weights set to 'global' or 'uniform'."
        )
