...
from nte.experiment.utils import save_timeseries_mul
...
class SHAPSaliency(Saliency):
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        save_timeseries_mul(mask=norm_mask, raw_mask=norm_mask, time_series=data,
...
class LimeSaliency(Saliency):
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        norm_mask = (mask - np.min(mask)) / (np.max(mask) - np.min(mask))
        save_timeseries_mul(mask=norm_mask, raw_mask=mask, time_series=data.flatten(),
                        save_dir=kwargs['save_dir'],
...
from nte.models.saliency_model.counterfactual_multi_ori import CFExplainer0
