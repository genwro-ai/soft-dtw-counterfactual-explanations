...
from nte.experiment.utils import tv_norm, save_timeseries_mul
from nte.models.saliency_model import Saliency

class CFExplainer0(Saliency):
    def __init__(self, background_data, background_label, predict_fn, enable_wandb, use_cuda, args):
        super(CFExplainer0, self).__init__(background_data=background_data,
    ...
    def generate_saliency(self, data, label, **kwargs):
        ...
        # assert mask.shape.numel() == data.shape
        ...
        max_iterations_without_improvement = 100  # Define the maximum number of iterations without improvement
        imp_threshold = 0.001
        # cf_prob = float('-inf')
        ...
        while i <= self.args.max_itr:
            ...
            pred_outputs = self.softmax_fn(self.predict_fn(perturbated_input.reshape(1, perturbated_input.shape[0], perturbated_input.shape[1]).float()))
        ...
        ori_perturbated = perturbated_input.cpu().detach().numpy()
        ...
        save_timeseries_mul(mask=flatten_mask, raw_mask=None,
                        time_series=data.cpu().detach().numpy(),
                        perturbated_output=perturbated_input.cpu().detach().numpy(),
                        save_dir=kwargs['save_dir'],
                        enable_wandb=self.enable_wandb, algo=self.args.algo,
                        dataset=self.args.dataset,
                        category=top_prediction_class)

        return mask, ori_perturbated, target_prob
